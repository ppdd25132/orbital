...
function parseMessageBody(payload) {
  const part = findBodyPart(payload, ["text/html", "text/plain"]);
  ...
  const entries = await Promise.all(
    descriptors.map(async (descriptor) => {
      try {
        ...
        const dataUri = `data:${descriptor.mimeType};base64,${normalizeBase64Url(data)}`;
        const keys = [descriptor.contentId, descriptor.filename, descriptor.attachmentId].filter(
          Boolean
        );

        return keys.map((key) => [key, dataUri]);
      } catch {
        return [];
      }
    })
