...
function collectInlineDescriptors(payload, descriptors = []) {
  ...
  for (const part of payload.parts) {
    ...
    // Some mailers (e.g. Anthropic transactional email) embed images directly
    // in the MIME body (body.data, no attachmentId) rather than as separate
    // attachment parts.  We include both variants so they can be resolved.
    const inlineData = part.body?.data;
    const isInline = Boolean(contentId) || /inline/i.test(disposition || "");

    if (isInline && (attachmentId || inlineData)) {
      descriptors.push({
        attachmentId,
        inlineData,     // base64url-encoded image data (may be absent)
        contentId,
    ...
    descriptors.map(async (descriptor) => {
      try {
        ...
        if (descriptor.attachmentId) {
          ...
          // The image bytes are already embedded in the MIME part body
          // (base64url-encoded), so no extra API call is needed.
          rawData = descriptor.inlineData;
        }

        if (!rawData) return [];

        // Normalize to standard padded base64 for reliable data: URL rendering
        let b64 = normalizeBase64Url(rawData);
